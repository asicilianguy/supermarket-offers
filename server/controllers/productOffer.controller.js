...
export const batchInsertOffers = async (req, res) => {
  try {
    ...
    const validatedProducts = productsInfo.map((product) => {
      // Ensure supermarketAisle is an array and contains valid values
      if (!Array.isArray(product.supermarketAisle)) {
        product.supermarketAisle = []
      }

      // Filter only valid aisles
      product.supermarketAisle = product.supermarketAisle.filter((aisle) => VALID_AISLES.includes(aisle))

      // Convert date strings to Date objects
      if (product.offerStartDate) {
        const [day, month, year] = product.offerStartDate.split("-")
        product.offerStartDate = new Date(`${year}-${month}-${day}`)
      }

      if (product.offerEndDate) {
        const [day, month, year] = product.offerEndDate.split("-")
        product.offerEndDate = new Date(`${year}-${month}-${day}`)
      }
    ...
    const insertedOffers = await ProductOffer.insertMany(validatedProducts, {
      ordered: false, // Continue inserting even if some fail
    })

    res.json({
      success: true,
      message: `Successfully inserted ${insertedOffers.length} offers`,
      insertedCount: insertedOffers.length,
      failedCount: productsInfo.length - insertedOffers.length,
    })
  } catch (err) {
    console.error("Error in batchInsertOffers:", err.message)
    ...
    if (err.code === 11000) {
      ...
      return res.status(207).json({
        success: false,
        message: "Some offers already exist",
        insertedCount: inserted,
        failedCount: req.body.productsInfo.length - inserted,
        error: err.message,
      })
    }
